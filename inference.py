"""
Inference Script for Hackathon Env
==================================
...
# Add envs/ to path for imports - works both locally and in Docker
_envs_dir = Path(__file__).parent / "envs"
if _envs_dir.exists():
    sys.path.insert(0, str(_envs_dir))

from hackathon_env import HackathonEnv
from hackathon_env.models import LogiChainAction
...
LOCAL_IMAGE_NAME = os.getenv("LOCAL_IMAGE_NAME", "hackathon-env:latest")

BENCHMARK = "hackathon_env"
MAX_STEPS = 50
TEMPERATURE = 0.7
MAX_TOKENS = 512
...
async def run_task(client: OpenAI, task_name: str) -> Dict:
    ...
    env = await HackathonEnv.from_docker_image(LOCAL_IMAGE_NAME)
